    ...
    if infeas <= feasTol  % should always be feasible
        ...
        if ~calcMetMwRange
            ...
            if MWele(jEC) > 0
                ...
                [infeasibility(jEC).minMw, infeasibility(jEC).maxMw] = deal(0);
            end
        end
    else
...
if ~calcMetMwRange
    ...
    metEle = cat(3, metEle, metEle);
    for jEC = 1:nEC
        switch stat{jEC}
            case 'minIncon'
                metEle(metU, eleConnect(:,jEC), 1) = metEleU.minIncon(:, eleConnect(:,jEC));
                metEle(metU, eleConnect(:,jEC), 2) = metEleU.minIncon(:, eleConnect(:,jEC));
            case 'minMw'
                metEle(metU, eleConnect(:,jEC), 1) = metEleU.minMw(:, eleConnect(:,jEC));
                metEle(metU, eleConnect(:,jEC), 2) = metEleU.minMw(:, eleConnect(:,jEC));
            case 'maxMw'
                metEle(metU, eleConnect(:,jEC), 1) = metEleU.maxMw(:, eleConnect(:,jEC));
                metEle(metU, eleConnect(:,jEC), 2) = metEleU.maxMw(:, eleConnect(:,jEC));
            case 'minMw & maxMw'
                metEle(metU, eleConnect(:,jEC), 1) = metEleU.minMw(:, eleConnect(:,jEC));
                metEle(metU, eleConnect(:,jEC), 2) = metEleU.maxMw(:, eleConnect(:,jEC));
        end
...
if ~calcMetMwRange
    ...
    S_fill = [];
    elements = eleK;
    rxnBal.minMW = metEle(:, :, 1)' * model.S;  % reaction balance
    rxnBal.maxMW = metEle(:, :, 2)' * model.S;
    model = metMw;  % range for the MW of the metabolite of interest as the 1st output
end
