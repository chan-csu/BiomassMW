function [metForm, metMw, ele, metEle, rxnBal, S_fill, solInfo, varargout] ...
...
%      [metForm, metMw, ele, metEle, rxnBal, S_fill, solInfo, LP] = computeMetFormulae(model, 'parameter', value, ...)
%    Find the min/max possible MW of a particular metabolite of interest:
%      [metForm, metMw, ele, metEle, rxnBal, S_fill, solInfo, LP] = computeMetFormulae(model, 'metMwRange', met, ...)
%    Also support direct input in the following order:
%      [metForm, metMw, ele, metEle, rxnBal, S_fill, solInfo, LP] ...
...
%    metMwRange:     a met of interest or its ID for which the range of possible molecular weights is determined
%    LPparams:       Additional parameters for `solveCobraLP`. See `solveCobraLP` for details
%
% OUTPUTS:
%    model:          COBRA model with updated formulas
%    metCompute:     Formulae for unknown mets (#unknown mets x 2 cell array)
...
metInterest = parser.Result.metMwRange;
...
elseif ~isempty(metInterest) && any(metK == metI)
    fprintf('The met of interest (supplied in the argument ''metMwRange'') already has a known formula. Nothing to calculate.\n');
    metMwRange = repmat(MW(model.metFormulas(metI)), 1, 2);
    metForm = repmat(model.metFormulas(metI), 2, 1);
    [ele, metEle, rxnBal, S_fill, solInfo, LP] = deal([]);
    
    model = metMwRange;  % metMwRange as the 1st output
    metCompute = metForm;  % metForm as the 2nd output
    varargout = {LP};
    return
end
...
for jEC = 1:nEC
    ...
    % no need to reconstruct the problem matrix if both the sets of elements
    % solved in the current and the previous round are singletons with no metabolites for filling inconsistency.
    if jEC == 1 || kE > 1 || kE0 > 1 || mFC > 0 || mFC0 > 0
        LPj = struct();
        kE0 = kE;
        mFC0 = mFC;
    ...
    if infeas <= feasTol  % should always be feasible
        ...
        if isempty(metInterest)
            ...
            if ~isnan(cMW(j)) && cMW(j) > 0
                LPj.c(metI) = cMW(j);
                % minimization, adjust tolerance if infeasible
                f = 1e-6;
                while true
                    if nargin < 6
                        sol(jEC).minMw = solveCobraLP(LPj);
                    else
                ...
                while true
                    if nargin < 6
                        sol(jEC).maxMw = solveCobraLP(LPj);
                    else
...
elseif isempty(metInterest) && all(strcmp(stat, 'minForm'))
    solInfo.stat = 'minForm';
elseif isempty(metInterest) && all(strcmp(stat, 'minFill'))
    solInfo.stat = 'minFill';
elseif ~isempty(metInterest) && all(strcmp(stat, 'minMw'))
    solInfo.stat = 'minMw';
elseif ~isempty(metInterest) && all(strcmp(stat, 'maxMw'))
    solInfo.stat = 'maxMw';
elseif ~isempty(metInterest) && all(strcmp(stat, 'minMw & maxMw'))
    ...
    if isempty(metInterest) 
        ...
        solInfo.ele = ele;
        solInfo.N = N;
        metForm = [model.mets(metU) model.metFormulas(metU)];
        metMw = MW(metForm(:, 2));
    else
        ...
        metForm = convertMatrixFormulas(eleK, [metEleU.minMw(metIinU, :); metEleU.maxMw(metIinU, :)], 10);
        metForm = [repmat(model.mets(metI), 2, 1), metForm];
        S_fill = [];
    end
    % reaction balance
